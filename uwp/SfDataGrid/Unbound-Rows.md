...
N> `AllowFrozenGroupHeaders` is not supported.
...
You can get the details view data grid using `GridUnBoundRowEventsArgs.OriginalSender` of the `QueryUnBoundRow` event, which fired the event and rendered in UI. 
