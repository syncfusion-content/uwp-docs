...
- **Expert Component Knowledge** - Deep understanding of 90+ UWP components and their implementation patterns.
...
You can install the Syncfusion UWP MCP server as a local tool without a global installation. For guidance on installing and managing local .NET tools, refer to the [documentation](https://learn.microsoft.com/en-us/dotnet/core/tools/local-tools-how-to-use).

1. Install the Syncfusion UWP MCP tool locally:

    ````bash
    dotnet tool install Syncfusion.UWP.MCP
    ````
    ...
    ````json
    {
      "servers": {
        "sf-uwp-mcp": {
          "type": "stdio",
          "command": "dotnet",
          "args": ["tool", "run", "syncfusion-uwp-mcp"],
          "env": {
            "Syncfusion_API_Key_Path": "YOUR_API_KEY_FILE_PATH"
            // or
            // "Syncfusion_API_Key": "YOUR_API_KEY"
...
**Verifying Installation** Check your editor's MCP Server list for `sf-uwp-mcp` with a **Connected** status to confirm a successful installation.

## Common use cases

The examples below showcase how the `search_docs` tool handles real-world UWP development scenarios. The tool can be invoked directly, as shown in the examples below, for specific needs. Alternatively, an AI assistant can automatically invoke it based on the request.
...
#search_docs Create a Syncfusion UWP Grid with paging, sorting, and filtering enabled.
{% endpromptcard %}
{% endpromptcards %}

{% promptcards %}
{% promptcard Scheduler Views Setup %}
#search_docs How do I configure the Syncfusion WPF Scheduler to support Day, Week,Month, TimelineDay, TimelineWeek, TimelineWorkWeek, and TimelineMonth views?
{% endpromptcard %}
{% endpromptcards %}
...
#search_docs How do I enable recurring events and resource grouping in the Schedule component?
{% endpromptcard %}
{% endpromptcards %}
...
#search_docs Why isn’t my DataGrid exporting to PDF or Excel correctly?
{% endpromptcard %}
{% endpromptcards %}

{% promptcards %}
{% promptcard Calendar Issues %}
#search_docs Why isn’t my Calendar selecting the correct date range?
{% endpromptcard %}
{% endpromptcards %}

## Best Practices

To get the most out of the Syncfusion<sup style="font-size:70%">&reg;</sup> UWP MCP Servers:

- **Be Specific** - Include the platform and component in your queries (for example, *"Create a Syncfusion UWP Grid with sorting and filtering"*).
...
- **Stay Consistent** - Keep file organization, naming conventions, and coding standards consistent throughout your UWP project.
...
- **For Troubleshooting** - Use AI suggestions for common issues; consult the [official documentation](https://help.syncfusion.com/uwp/overview) or [support](https://support.syncfusion.com/support/tickets/create) for complex problems.
