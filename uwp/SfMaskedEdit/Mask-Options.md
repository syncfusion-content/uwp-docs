...
Each `MaskType` has different sets of mask elements combined to form a mask expression. Based on the complexity and usage, mask types are classified as:
...
For example, for input, “\d\w*\d” – The output can be “12” or “1a1” or “123”, because the “\w*” is optional. If the user types the input “1a” the character, "a" will match with optional mask and the Masked text will be “1a_”. If the mask text is “12” and if the character “3” is typed, the character “2” will be considered as optional mask and the masked text will be “123”.
...
“(?=regex)” is a positive lookahead regex. It matches only when the regex is matched with the input text.

For example, for input, “(?=123)\d{3}” allows the first two digits and while entering the third digit it will validate the regex and restrict the input. It will allow the output only when the MaskText is “123” for this given input.
...
“(?!regex)” is a negative lookahead regex. It validates the input on every keypress.

For example, for input, “(?!55)(?!000)(?!666)\d{3}” – \d{3} denotes that the output will be of 3 character length, “(?55)” denotes the value will not begin with “55” and the “(?!000)(?!666)” denotes the output cannot be “000” or “666”.
...
<td>
(a|b)+</td><td>
Accepts the matching input with one or more occurrences.</td></tr>
<tr>
<td>
[ABC]</td><td>
...
<td>
[^ABC]</td><td>
Accepts any single character which is not included in the specified set of characters.</td></tr> 
<tr>
<td>
[A-Z]</td><td>
...
<td>
\W</td><td>
Accepts an  non-word character. \W is the same as [^a-zA-Z_0-9].</td></tr>
<tr>
<td>
\s</td><td>
...
<td>
{n,}</td><td>
Accepts input for 'n' and more than 'n' number of times. </td></tr>
<tr>
<td>
{n,m}</td><td>
Accepts input for n minimum number of times and m maximum number of times.</td></tr>
...
For example, following code example shows the validation for E-mail using Regex elements.
