...
1. Open the Syncfusion UWP offline installer file from the downloaded location by double-clicking it. The Installer Wizard automatically opens and extracts the package.
...
7. Click the **Finish** button. Your system has now been installed with the Syncfusion Essential Studio UWP product.
...
2. The file `syncfusionessentialuniversalwindows_{version}.exe` will be extracted into the Temp directory.
3. Run `%temp%`. The Temp folder will open. The `syncfusionessentialuniversalwindows_{version}.exe` file will be located in one of the folders.
4. Copy the extracted `syncfusionessentialuniversalwindows_{version}.exe` file to a local drive.
5. Exit the Wizard.
